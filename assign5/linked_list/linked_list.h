...
public:
  int get_length();
  // note: there is no set_length(unsigned int) (the reasoning should be
  // intuitive)

  void print(); // output a list of all integers contained within the list
  void
  clear(); // delete the entire list (remove all nodes and reset length to 0)
  ...
  void
  insert(int val,
         int index); // insert a new value in the list at the specified index
  ...
  void sort_ascending(); // sort the nodes in ascending order. You must
                         // implement the recursive Merge Sort algorithm
  // Note: it's okay if sort_ascending() calls a recursive private function to
  // perform the sorting.
