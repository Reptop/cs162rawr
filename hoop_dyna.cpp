...
void endGame(char *arr[MAX_SIZE], bool &running) {

  // delete array from inside out
...
void runGame(char *arr[MAX_SIZE], int scrubbedInput1, int scrubbedInput2,
             int &playerTotal1, int &playerTotal2) {

  playTurn(arr, 1, scrubbedInput1, playerTotal1);
  playTurn(arr, 2, scrubbedInput2, playerTotal2);
}

void initArr(char *arr[MAX_SIZE]) {
  for (int i = 0; i < MAX_SIZE; ++i)
    arr[i] = new char[MAX_SIZE];
}
...
int main() {
  ...
  // Double pointer char so I can create a dynamic array of characters
  // We are essentially making a jagged array
  char **rack1 = new char *[MAX_SIZE];
  ...
  while (running) {
    ...
    if (scrubbedInput1 == -1 || scrubbedInput2 == -1)
      ...
      runGame(rack1, scrubbedInput1, scrubbedInput2, playerTotal1,
              playerTotal2);
      checkWin(playerTotal1, playerTotal2);

      if (!playAgain()) {
        endGame(rack1, running);
      }
